...
class Block35(nn.Module):
    def __init__(self, scale, input_channels):
        ...
        self.conv2d = nn.Conv2d(
            conv3_2 + 64, input_channels, kernel_size=1, stride=1)
        self.relu = nn.ReLU(inplace=False)
...
class Block17(nn.Module):
    def __init__(self, scale, input_channels, m):
        ...
        self.branch0 = BasicConv2d(
            input_channels, m // 2, kernel_size=1, stride=1)
        ...
        self.branch1 = nn.Sequential(
            ...
            BasicConv2d(
                128, step1, kernel_size=(1, 7), stride=1, padding=(0, 3)),
            BasicConv2d(
                step1, m // 2, kernel_size=(7, 1), stride=1, padding=(3, 0)))
...
class Mixed_7a(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.branch2 = nn.Sequential(
            BasicConv2d(input_channels, 256, kernel_size=1, stride=1),
            BasicConv2d(
                256, channels_middle, kernel_size=3, stride=1, padding=1),
            BasicConv2d(
                channels_middle, output_channels, kernel_size=3, stride=2))
...
class Block8(nn.Module):
    def __init__(self, scale, input_channels, ll, noReLU=False):
        ...
        self.branch0 = BasicConv2d(
            input_channels, 192, kernel_size=1, stride=1)

        step1 = (ll + 192) // 2
        self.branch1 = nn.Sequential(
            BasicConv2d(input_channels, 192, kernel_size=1, stride=1),
            BasicConv2d(
                192, step1, kernel_size=(1, 3), stride=1, padding=(0, 1)),
            BasicConv2d(
                step1, ll, kernel_size=(3, 1), stride=1, padding=(1, 0)))

        self.conv2d = nn.Conv2d(
            192 + ll, input_channels, kernel_size=1, stride=1)
        if not self.noReLU:
            self.relu = nn.ReLU(inplace=False)
...
class InceptionResNet(nn.Module):
    def __init__(self,
        ...
        self.mixed_6a = Mixed_6a(input_channels=channel35, k=k, ll=l_index, m=m, n=n)
        ...
        self.mixed_7a = Mixed_7a(
            input_channels=repeat1_input, output_channels=channel35)

        repeat2_input = repeat1_input + channel35 * 3 // 2 + 512
        sequence_list2 = []
        for j in range(num_repeat[2]):
            sequence_list2.append(
                Block8(scale=scale[2], input_channels=repeat2_input, ll=l_index))
        self.repeat2 = nn.Sequential(*sequence_list2)

        self.block8 = Block8(
            scale=scale[2], input_channels=repeat2_input, ll=l_index, noReLU=True)
        self.conv2d_7b = BasicConv2d(
            repeat2_input, 1536, kernel_size=1, stride=1)
...
def inception_resnet_v1(**kwargs):
    model = InceptionResNet(
        channel35=256, k=192, l_index=192, m=256, n=384, **kwargs)
    return model


def inception_resnet_v2(**kwargs):
    model = InceptionResNet(
        channel35=320, k=256, l_index=256, m=384, n=384, **kwargs)
    return model
